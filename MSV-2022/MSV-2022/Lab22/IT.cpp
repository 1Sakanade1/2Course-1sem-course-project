			...
			*(idStream) << std::setw(15) << "Идентификатор:" << std::setw(17) << "Тип данных:" << std::setw(15) << "Значение:" << std::setw(27) << "Длина строки:" << std::setw(24) << "Первое вхождение:" << std::endl;
			...
			for (int i = 0; i < this->size; i++)
			{
				if (this->table[i].idtype == IT::IDTYPE::L)
				{
					...
					case 1:
					{
						*(idStream) << "   "  << "\t\t\t\t" << "INT " << "\t\t" << this->table[i].value.vint << "\t\t" << "-\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					case 2:
					{
						*(idStream) << "   "  << "\t\t\t\t" << "STR " << " \t   " << this->table[i].value.vstr.str << std::setw(30 - strlen(this->table[i].value.vstr.str)) << (int)this->table[i].value.vstr.len << "\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					}
				}
			...
			*(idStream) << std::setw(15) << "Идентификатор:" << std::setw(26) << "Тип данных возврата:" << std::setw(36) << "Количество переданных параметров:" << std::setw(22) << "Первое вхождение:" << std::endl;
			...
			for (int i = 0; i < this->size; i++)
			{
				if (this->table[i].idtype == IT::IDTYPE::F)
				{
					...
					case 1:
					{
						*(idStream) << "   " << this->table[i].id << std::setw(38 - strlen(this->table[i].id)) << "INT " << "\t\t\t\t" << this->table[i].parmQuantity << "\t\t\t\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					case 2:
					{
						*(idStream) << "   " << this->table[i].id << std::setw(38 - strlen(this->table[i].id)) << "STR " << "\t\t\t\t" << this->table[i].parmQuantity << "\t\t\t\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					}
			...
			*(idStream) << std::setw(25) << "Имя родительского блока:" << std::setw(20) << "Идентификатор:" << std::setw(16) << "Тип данных:" << std::setw(24) << "Тип идентификатора:" << std::setw(21) << "Первое вхождение:" << std::endl;
			...
			for (int i = 0; i < this->size; i++)
			{
				if (this->table[i].idtype == IT::IDTYPE::V)
				{
					...
					case 1:
					{
						*(idStream) << "   " << this->table[i].parrent_function << std::setw(60 - strlen(this->table[i].parrent_function)) << this->table[i].id << std::setw(32) << "INT " << std::setw(20) << "V" << "\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					case 2:
					{
						*(idStream) << "   " << this->table[i].parrent_function << std::setw(60 - strlen(this->table[i].parrent_function)) << this->table[i].id << std::setw(32) << "STR " << std::setw(20) << "V" << "\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					}
				...
				if (this->table[i].idtype == IT::IDTYPE::P)
				{
					...
					case 1:
					{
						*(idStream) << "   " << this->table[i].parrent_function << std::setw(60 - strlen(this->table[i].parrent_function)) << this->table[i].id << std::setw(32) << "INT " << std::setw(20) << "P" << "\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					case 2:
					{
						*(idStream) << "   " << this->table[i].parrent_function << std::setw(60 - strlen(this->table[i].parrent_function)) << this->table[i].id << std::setw(32	) << "STR " << std::setw(20) << "P" << "\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					}
